...
class GRUGCNEncoder(nn.Module):
    def __init__(self, args: Namespace, atom_fdim: int, mol_fdim: int):
        ...
        #self.update_layer = nn.LSTMCell(atom_fdim, atom_fdim)
    ...
    def readout(self, h, h0, step):
        ...
        return torch.sum(torch.sigmoid(self.i_layers[index](torch.cat([h, h0], dim=2))) * self.j_layers[index](h), dim=1) #sum mean
...
class GRUGCN(nn.Module):
    def __init__(self, args: Namespace, graph_input: bool = False):
        ...
        self.encoder = GRUGCNEncoder(args, atom_fdim=args.hidden_size, mol_fdim=args.hidden_size)
