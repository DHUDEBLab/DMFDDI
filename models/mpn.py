...
##用于编码分子的消息传递神经网络--Encoder部分
class MPNEncoder(nn.Module):
    def __init__(self, args: Namespace, atom_fdim: int, bond_fdim: int):
        """Initializes the MPNEncoder.
        :param args: Arguments. 参数
        :param atom_fdim: Atom features dimension.原子特征维度
        :param bond_fdim: Bond features dimension.键特征维度
        """
        super(MPNEncoder, self).__init__()
        self.atom_fdim = atom_fdim #133
        self.bond_fdim = bond_fdim #147
        self.hidden_size = args.hidden_size #300 Dimensionality of hidden layers in MPN MPN中隐藏层的维度
        self.bias = args.bias #default=True action='store_true', default=True
        self.depth = args.depth #3层 3步 Number of message passing steps
        self.dropout = args.dropout #default=0.3
        self.layers_per_message = 1
        self.undirected = args.undirected #聚合两原子间的边 无向边（总是将两个相关的键向量相加） FALSE default=False, help='Undirected edges (always sum the two relevant bond vectors)'
        self.atom_messages = args.atom_messages #仅考虑在原子上的消息传递 Use messages on atoms instead of messages on bonds
        self.features_only = args.features_only #仅使用 FFN 中的附加功能，不使用图网络 Use only the additional features in an FFN, no graph network
        self.use_input_features = args.use_input_features #输入的特征 有的话512 无None
        self.args = args
        ...
        # Dropout
        self.dropout_layer = nn.Dropout(p=self.dropout) #p=0.3

        # Activation
        self.act_func = get_activation_function(args.activation) #RELU

        # Cached zeros 缓存零
        self.cached_zero_vector = nn.Parameter(torch.zeros(self.hidden_size), requires_grad=False) #一种被视为模块参数的张量。

        # Input
        input_dim = self.atom_fdim if self.atom_messages else self.bond_fdim #bond_fdim 147
        self.W_i = nn.Linear(input_dim, self.hidden_size, bias=self.bias) #Applies a linear transformation to the incoming data: :math:`y = xA^T + b`

        if self.atom_messages: #false
            w_h_input_size = self.hidden_size + self.bond_fdim
        else:
            w_h_input_size = self.hidden_size #300

        # Shared weight matrix across depths (default) 跨深度共享权重矩阵（默认）
        ...
        self.attn_num_d = self.args.attn_num_d #Number of units in attention weight parameters 1.
        self.attn_num_r = self.args.attn_num_r #Number of units in attention weight parameters 2
        ...
        self.softmax = nn.Softmax(dim=1) #将 Softmax 函数应用于 n 维输入张量，重新缩放它们，使 n 维输出张量的元素位于 [0,1] 范围内并且总和为 1
    ...
    def forward(self,
        ...
        """
        Encodes a batch of molecular graphs. 编码一批分子图。
        :param mol_graph: A BatchMolGraph representing a batch of molecular graphs. mol_graph：表示一批分子图的 BatchMolGraph。
        :param features_batch: A list of ndarrays containing additional features. features_batch：包含附加功能的 ndarray 列表。
        :return: A PyTorch tensor of shape (num_molecules, hidden_size) containing the encoding of each molecule. 一个形状为 (num_molecules, hidden_​​size) 的 PyTorch 张量，其中包含每个分子的编码。
        """
        ...
        f_atoms, f_bonds, a2b, b2a, b2revb, a_scope, b_scope = mol_graph.get_components() #一个包含 PyTorch 张量的元组，具有原子特征、键特征和图形结构，以及两个指示原子和键范围的列表（即它们属于哪些分子）
        ...
        # Input
        if self.atom_messages: #使用关于原子的消息而不是关于键的消息 false
        ...
        for depth in range(self.depth - 1):
            ...
            if self.atom_messages:
                ...
                nei_a_message = index_select_ND(message, a2b)  # num_atoms x max_num_bonds x hidden 从与 index 中的原子或键索引对应的源中选择消息特征。
...
class MPN(nn.Module):
    def __init__(self,
        ...
        :param atom_fdim: Atom features dimension.原子特征维度
        :param bond_fdim: Bond features dimension.键特征维度
        :param graph_input: If true, expects BatchMolGraph as input. Otherwise expects a list of smiles strings as input.如果为真，则期望BatchMolGraph作为输入。 否则需要一个字符串列表作为输入
        """
        super(MPN, self).__init__()
        self.args = args
        self.atom_fdim = atom_fdim or get_atom_fdim(args) #在featurization文件中 def get_atom_fdim(args: Namespace) -> int: 获取原子特征的维度。
    ...
    def forward(self,
        ...
        """
        # 编码一批分子SMILES字符串。
        # :param batch：SMILES字符串列表或BatchMolGraph（如果self.graph_input为True）。
        # :param features_batch：包含附加功能的ndarray列表。
        # :return: 一个形状为(num_molecules, hidden_size) 的PyTorch张量，其中包含每个分子的编码。
