...
def index_select_ND(source: torch.Tensor, index: torch.Tensor) -> torch.Tensor:
    """
    Selects the message features from source corresponding to the atom or bond indices in index.从与 index 中的原子或键索引对应的源中选择消息特征。
...
def get_activation_function(activation: str) -> nn.Module:
    ...
    :return: The activation function module. 激活函数模块。
    """
    if activation == 'ReLU':
        return nn.ReLU()
...
class NoamLR(_LRScheduler):
    """
    具有分段线性增加和指数衰减的 Noam 学习率调度器。

     在整个过程中，学习率从 init_lr 线性增加到 max_lr
     第一个warmup_steps（其中warmup_steps = warmup_epochs * steps_per_epoch）。
     然后学习率从 max_lr 到 final_lr 呈指数下降
     剩余 total_steps 的过程 -warmup_steps（其中 total_steps =
     total_epochs * steps_per_epoch）。 这大致是基于学习率
     来自“注意力就是你所需要的一切”的时间表，第 5.3 节 (https://arxiv.org/abs/1706.03762)。
    """
    def __init__(self,
                 optimizer: Optimizer,
