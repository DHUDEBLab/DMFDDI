...
def get_task_names(path: str, use_compound_names: bool = False) -> List[str]:
    ...
    从数据 CSV 文件中获取任务名称。
     ...
     :param use_compound_names: 除了微笑字符串之外，文件是否还有复合名称。
     :return: 任务名称列表。
    """
...
def get_smiles(path: str, header: bool = True) -> List[str]:
    """
    从数据 CSV 文件中返回微笑字符串（假设第一行是标题）。

     :param path: CSV 文件的路径。
     :param header: CSV 文件是否包含标题（将被跳过）。
     :return: 微笑字符串列表。
    """
    with open(path) as f:
        reader = csv.reader(f)
...
def filter_invalid_smiles(data: MoleculeDataset) -> MoleculeDataset:
    ...
    过滤掉无效的 SMILES。
     ...
     :param data: 一个分子数据集。
     :return: 一个只有有效分子的 MoleculeDataset。
    """
...
def get_data(path: str,
    ...
    """
    从 CSV 文件中获取微笑字符串和目标值（以及可选的复合名称，如果提供）。

     :param path: CSV 文件的路径。
     :param skip_invalid_smiles: 是否跳过和过滤掉无效的微笑。
     :param args: 参数。
     :param features_path：包含功能的文件的路径列表。 如果提供，则使用
     代替 args.features_path。
     :param max_data_size: 要加载的最大数据点数。
     :param use_compound_names: 除了微笑字符串之外，文件是否还有复合名称。
     :param 记录器：记录器。
     :return: 一个包含微笑字符串和目标值的 MoleculeDataset
     需要时提供其他信息，例如附加功能和复合名称。
    """
