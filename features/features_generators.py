...
def register_features_generator(features_generator_name: str) -> Callable[[FeaturesGenerator], FeaturesGenerator]:
    """
    Registers a features generator.注册一个特征生成器。
    ...
    """
    # 注册一个特征生成器。
    # :param features_generator_name：调用FeaturesGenerator的名称。
    # :return: 一个装饰器，它将使用指定的名称将FeaturesGenerator添加到注册表中。
...
def get_features_generator(features_generator_name: str) -> FeaturesGenerator:
    ...
    """
    # 按名称获取已注册的 FeaturesGenerator。
    # :paramfeatures_generator_name：FeaturesGenerator的名称。
    # :return: 所需的FeaturesGenerator。
...
def get_available_features_generators() -> List[str]:
    """Returns the names of available features generators."""#返回可用特征生成器的名称。
...
def morgan_binary_features_generator(mol: Molecule,
    ...
    """
    Generates a binary Morgan fingerprint for a molecule.为分子生成二进制摩根指纹。
...
"""
Custom features generator template.自定义特征生成器模板。
